...
module.exports = function autoscaling(AwsAutoscaling, AwsEcs) {
  ...
  function startEcsContainer(params, cb) {
    ...
    autoscaling.setDesiredCapacity({
      ...
      if (err) {
        ...
        log.info(`Successfully set AutoScaling group ${AutoScalingGroupName} to ${DesiredCapacity}.`)

        async.during(function (callback) {
          clusterContainerCountIsZero({ cluster: params.Cluster }, callback)
        }, function (callback) {
  ...
  function clusterContainerCountIsZero(params, cb) {
    log.info(`Polling ECS cluster ${params.cluster} to check container count is positive`)
    ecs.describeClusters({
      ...
      if (err) {
        ...
        var containerCount = data.clusters[0].registeredContainerInstancesCount
